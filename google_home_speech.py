...
def get_local_ip():
        ...
        for iface_name in netifaces.interfaces():
                ...
                if debug: print("local_ip = " + ipv4_addr)
                return ipv4_addr
        if debug: print("failed to get local_ip")
        return ''                


def discovery_google_home():
        if debug : print("Start discovery google home...")
        chromecasts = pychromecast.get_chromecasts()
        if debug : print(chromecasts)
        for cc in chromecasts:
                if cc.device.friendly_name.startswith(config.friendly_name):
                        if debug : print("google home found friendly_name = " + cc.device.friendly_name)
                        cast = cc
...
def do_speech(text, language):
        ...
        if debug: print('get speech data for ' + text + '(' + language + ')')
        tts = gtts.gTTS(text = text, lang = language)
        tts.save(speech_filename)
        if debug: print('send speech data to google home')
        ...
        if debug: print('media_url = ' + media_url)
...
class JsonResponseHandler(BaseHTTPRequestHandler):
        def do_POST(self):
                ...
                if debug: print(self.headers)
                content_len = int(self.headers.get('content-length'))
                requestBody = self.rfile.read(content_len).decode('UTF-8')
                if debug: print('requestBody=' + requestBody)
                try:
                        jsonData = json.loads(requestBody)
                        if debug: print('**JSON**')
                        if debug: print(json.dumps(jsonData, sort_keys=False, indent=4, separators={',', ':'}))
                except:
                        if debug: print('faild to decode json')
                        jsonData = None
        ...
        def do_GET(self):
                path = self.path
                if debug: print ("GET requested for ", path)
                filename = path[1:]

                if filename != speech_filename:
                        if debug: print("deny GET request for " + path)
...
if debug: 
        ...
        print("")
        print("examples:")
        print('curl -X POST -d \'{"text":"test"}\' http://localhost:' + str(config.listen_port)  + '/')
        print('curl -X POST -d \'{"text":"Hello, this is test.","language":"en"}\' http://localhost:' + str(config.listen_port)  + '/')
        print('curl -X POST -d \'{"text":"你好","language":"zh-cn"}\' http://localhost:' + str(config.listen_port)  + '/')
        print("")
